#!/bin/bash
# ========================================
#  Y2Back Deploy Manager v3.2.2 - macOS
# ========================================
...
# Colores (compatible con Terminal.app de macOS)
RESET='\033[0m'
CYAN='\033[0;36m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
...
prueba_web() {
    ...
    find_port() {
        ...
        while true; do
            ...
            if ! lsof -i:$random_port >/dev/null 2>&1 && ! netstat -an 2>/dev/null | grep -q "\.$random_port "; then
                echo $random_port
                return 0
            fi
        done
    ...
    echo -e "${BLUE}ℹ️  Safari abrirá automáticamente${RESET}"
    echo
...
prueba_ambos() {
    ...
    echo
    echo -e "${BLUE}💡 Tip: Usa Cmd+T para abrir nueva pestaña en Terminal${RESET}"
    echo
...
menu_deploy() {
    ...
    echo -e "${GREEN}[3]${RESET} macOS Server"
    echo -e "${GREEN}[4]${RESET} Volver al menú principal"
    echo
...
deploy_linux() {
    ...
    echo "   2. Copiar archivos al servidor (scp/rsync)"
...
deploy_windows() {
    ...
    echo -e "${BLUE}📋 Próximos pasos:${RESET}"
    echo "   1. Generar bundle de deployment"
    echo "   2. Copiar archivos al servidor (WinSCP/RDP)"
    echo "   3. Instalar dependencias (npm install --production)"
...
deploy_macos() {
    ...
    echo -e "${CYAN}"
    echo "========================================"
    echo "  🍎 Deploy para macOS Server"
    echo "========================================"
    echo -e "${RESET}"
    echo
    echo -e "${BLUE}📦 Binarios requeridos:${RESET}"
    echo "   - yt-dlp_macos (Universal: x64 + arm64)"
    echo "   - ffmpeg_macos (Universal: x64 + arm64)"
    echo "   - ffprobe_macos (Universal: x64 + arm64)"
    echo
    echo -e "${BLUE}📋 Próximos pasos:${RESET}"
    echo "   1. Generar bundle de deployment"
    echo "   2. Copiar archivos al servidor (scp/airdrop)"
    echo "   3. Instalar dependencias (npm install --production)"
    echo "   4. Configurar launchd service (plist)"
    echo "   5. Dar permisos de ejecución (chmod +x)"
    echo "   6. Iniciar aplicación"
    echo
    echo -e "${BLUE}💡 Nota: Compatible con Intel y Apple Silicon${RESET}"
    echo
    echo -e "${YELLOW}ℹ️  Funcionalidad en desarrollo...${RESET}"
    echo
    read -p "Presiona Enter para volver al menú..."
